        ...
        private void ProcessFrame(object sender, EventArgs arg)
        {
            ...
            if (RetrieveBgrFrame.Checked)
            {
                ...
                else
                {
                    ...
                    CvInvoke.CalcOpticalFlowPyrLK(prev, frame11, Actual, new System.Drawing.Size(10, 10), 0, new MCvTermCriteria(20, 0.03d),
                             out nextFeature, out status, out trer);
                    imageToDisplay = Frame.ToImage<Bgr, byte>();
                    for (int x = 0; x < nextFeature.Length; x++)
                    {
                        imageToDisplay.Draw(new CircleF(new PointF(nextFeature[x].X, nextFeature[x].Y), 1f), new Bgr(Color.Blue), 2);
                    }
